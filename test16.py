...
def load_and_resize_image(file_path, width, height):
    ...
    resized_image = original_image.resize((width, height), resample=Image.BILINEAR)  # Utilisation de BILINEAR pour une interpolation lissée
...
class SensorApp:
    def __init__(self, master):
        ...
        self.action_button = tk.Button(master, text="Exécuter les actions", command=self.execute_actions)
    ...
    def execute_actions(self):
        ...
        if temperature_value > 25:
            print("Température élevée, arrosage nécessaire")
        else:
            print("Température acceptable, pas besoin d'arrosage")

        if humidity_value < 40:
            print("Humidité basse, arrosage nécessaire")
        else:
            print("Humidité acceptable, pas besoin d'arrosage")

        if luminosity_value < 5000:
            print("Faible luminosité, déplacer les plantes vers un endroit plus lumineux")
        else:
            print("Luminosité suffisante")
