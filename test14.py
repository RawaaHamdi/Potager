...
class PotagerApp:
    ...
    def on_message(self, client, userdata, msg):
     ...
     if topic == "/Thinkitive/hum":
        self.humidity.append(payload)
     if topic == "/Thinkitive/ldr":
    ...
    def update_sensor_labels(self):
        if self.temperature:
            self.temperature_label.config(text=f"Température: {self.temperature[-1]} °C")
        if self.humidity:
            self.humidity_label.config(text=f"Humidité: {self.humidity[-1]} %")
        if self.light:
            self.light_label.config(text=f"Luminosité: {self.light[-1]} lux")
